...
@section('content')
  ...
  <div class="container">
    ...
    <h1 class="title" style="width:100%;text-align:center;">Edit Article</h1>

    <form method="POST" action="/articles/{{$article->slug}}" enctype="multipart/form-data">
      {{ method_field('PATCH') }}
      {{ csrf_field() }}
      ...
      <div class="field" style="margin-left:175px;">
        ...
        <label class="checkbox" style="margin-right:25px;">
          <input type="checkbox" value="1" name="main_article" {{ $article->main_article ? 'checked' : ''}}> <b>Main Article</b>
        </label>

        <label class="checkbox {{ $article->top_headline ? 'is-complete' : '' }}">
          <input type="checkbox"  value="1" name="top_headline" {{ $article->top_headline ? 'checked' : ''}}> <b>Top Headline</b>
        </label>
          ...
          <div class="field">
            <div class="control">
              <textarea
                ...
                required>{{ $article->content_1 }}</textarea>
            </div>
          </div>
        </div>
      </div>
    ...
    <form action="/articles/{{ $article->slug }}" method="POST" style="margin-top:25px">
      @method('DELETE')
      @csrf
      <div class="field">
        <div class="control">
